...
function NimbusMethod({
  ...
  const [displayOptions, SetDisplayOptions] = useState<string>("all");
  const [instructionsMessage, SetInstructionsMessage] = useState<string>(
    "Classify the objective functions and click on iterate."
  );
  ...
  const handleDisplay = (event: React.ChangeEvent<HTMLInputElement>) => {
    SetDisplayOptions((event.target as HTMLInputElement).value);
    ...
    <Box sx={{ display: "flex", width: "-webkit-fill-available" }}>
        ...
        >
          ...
          <TabPanel value={valueTab} index={0}>
            <Row>
              ...
              <Typography sx={{ marginTop: "2rem" }}>{helpMessage}</Typography>
              <Box
                position="absolute"
                bottom="0px"
                display={"flex"}
      ...
      >
        ...
        <Container>
          ...
          <Card
            variant="outlined"
            sx={{ marginBottom: "1rem", backgroundColor: "#eeeeee" }}
          >
            <CardContent>
              <Typography sx={{ fontWeight: "bold" }}>Instructions:</Typography>
              <Typography>{instructionsMessage}</Typography>
            </CardContent>
          </Card>
